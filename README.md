...
SPA 和串口 CLI 不增加运行时依赖，V2/V3 BLE 协议保持不变。GitHub Actions 会运行 WebUi 契约、串口 CLI 契约、浏览器操作回归、BLE 失败路径测试、Native 测试和 ESP32 固件构建。浏览器操作测试需要 Node.js 18 或更新版本；Native 和 BLE 失败路径测试需要 PATH 中可用的 `gcc` / `g++`（Windows 可使用 MinGW）。
